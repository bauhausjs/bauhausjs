...
module.exports = function (bauhausConfig) {
    ...
    app.get('/:id', function (req, res, next) {
        ...
        db.getFile(id).then(function (data) {
            ...
            sendFile(res, noimage.buffer, noimage.metadata["content-type"]);
            /*console.error(err);
            res.writeHead(500);
    ...
    app.get('/asset/:id', function (req, res, next) {
        ...
        File.findOne({
            _id: id
        }, 'name metadata data', function (err, assetDocument) {
            var options;

            if (err) {
                return sendFile(res, noimage.buffer, noimage.metadata["content-type"]);
                assetDocument = noimage;
                fail = true;
            ...
            if (assetDocument && !assetDocument.data) {
                return sendFile(res, noimage.buffer, noimage.metadata["content-type"]);
                assetDocument = noimage;
                fail = true;
                //return next('No content');
            } // Asset has no content
            if (assetDocument && assetDocument.metadata && !assetDocument.metadata['content-type']) {
                return sendFile(res, noimage.buffer, noimage.metadata["content-type"]);
                assetDocument = noimage;
                fail = true;
                //return next('Missing Metadata');
            } // Asset has no metadata

            // temporary fix to avoid errors on non-images
            var isImage = /^image\//;
            if (assetDocument.metadata['content-type'].match(isImage) === null) {
                return sendFile(res, noimage.buffer, noimage.metadata["content-type"]);
                assetDocument = noimage;
                fail = true;
            ...
            sendAsset(res, assetDocument, options, fail);
        });
     ...
     */
    function sendAsset(res, assetDocument, options, fail) {
        var data,
            id;
        ...
        if (options && options.transform) {
            ...
            File.findOne(query, 'parentId transforms data', function (err, cachedAssetDocument) {
                if (!cachedAssetDocument) { //If no transformed asset exists with the given parameters
                    cachedAssetDocument = new File(); //we create a new one, and save it in the mongodb
